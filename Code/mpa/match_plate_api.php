...
if($status[0] == 1 && $status[1] == 1 && $status[2] == 1 && $status[3] == 1){
    include('store_in_QUERY.php'); //GETの不足部分を補完し、QUERYに保管
    include('get_data_by_height.php'); //高さを条件としてWP_QUERYを発行し、高さがマッチするプレートをPLATESに格納
    if(count($PLATES) != 0) include('matching_depth.php'); //段差の形状毎に、奥行き条件と照らし合わせ抽出
    if(count($PLATES) != 0) include('matching_width.php'); //横幅の条件と照らし合わせて抽出
    if(count($PLATES) != 0) include('sort_by_angle.php'); //配列を角度の緩やかな順に並び替え
}else{
    $PLATES = [];
}
